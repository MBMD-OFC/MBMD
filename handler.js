...
import { smsg } from './libreria/simple.js'
...
if (!('role' in user)) user.role = 'Debil'
...
premiumTime: 0, registered: false,
...
let texto = `- _El usuario nuevo que ha ingresado no es aceptable en este grupo, sera eliminado de inmediato._`
...
"mediaType": 1, 
sourceUrl: cafe }}}, { quoted: fkontak2 })
apii.data = ''
...
if (nk.isGroup == false) {
if (nk.status == "offer") {
let callmsg = await this.reply(nk.from, `❪ ✎ › _Hola usuario_ *@${nk.from.split('@')[0]}*, no esta permitido hacer ${nk.isVideo ? 'una video llamada' : 'una llamada'} en este chat, seras bloqueado.`, false, { mentions: [nk.from] })
