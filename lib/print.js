...
return chalk.bold(`${name_info ? name_info : 'Alguien' }`)
}).join(', ')

console.log(`
🝔 ${chalk.cyan('Bot »')} ${chalk.bgMagenta(chalk.white.italic('%s'))}
🝔 ${chalk.cyan('Horario »')} ${chalk.black.bgWhite('%s')}
🝔 ${chalk.green('Accion »')} ${await formatMessageStubType(m.messageStubType, name_user)}
🝔 ${chalk.green('Usuario »')} ${chalk.white('%s')}
🝔 ${chalk.green('Recursos » ')} ${chalk.cyan('%s%s')}
🝔 %s
🝔 ${chalk.redBright('Peso »')} ${chalk.blueBright('%s (%s %sB)')}
🝔 ${chalk.redBright('Tipo »')} ${chalk.greenBright.bold('[%s]')}
`.trim(),
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' servidor bot'}`,
(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString(),
sender,
m ? '' : '',
`sin registro`,
(m.chat.includes("@g.us") ? `🝔 ${chalk.green('Grupo »')} ${chalk.yellowBright.italic(`${chat}`)}` : `🝔 ${chalk.green('Privado »')} ${chalk.yellowBright.italic(`${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
await formatMessageTypes(m.mtype) || 'Not specified' //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
)
...
console.log(m.error != null ? chalk.red.bold(log) : m.isCommand ? chalk.green.bold(log) : log)
}
