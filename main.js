...
const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['MBMD', 'Safari', '2.0.0'] : methodCodeQR ? ['MBMD', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
    auth: {
        creds: state.creds,
...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'fileMBmd'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
...
try {
let listaDirectorios = readdirSync('./fileMBmd/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./fileMBmd/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./fileMBmd/${directorio}`).filter(fileInDir => {
...
if (reason == 405) {
await fs.unlinkSync("./SesionConectado/" + "creds.json")
