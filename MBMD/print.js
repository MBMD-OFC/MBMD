...
return chalk.bold(`${name_info ? name_info : mbmd.idioma_code === 'es' ? 'Alguien' : 'Someone' }`)
}).join(', ')

console.log(`
╭━━━━━━━━━━━━━━𖡼
┃ ❖ ${chalk.white.bold('Bot:')} ${chalk.cyan.bold('%s')} 
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Horario:' : 'Schedule:')} ${chalk.black.bgGreen('%s')}
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Acción:' : 'Action:')} ${mbmd.idioma_code === 'es' ? await formatMessageStubType(m.messageStubType, name_user) : await formaTxtStub(WAMessageStubType[m.messageStubType])}
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Usuario:' : 'User:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / Premium » ${user?.premiumTime > 0 ? '✅' : '❌'}
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Recursos:' : 'Resources:')} ${chalk.yellow('%s%s')}
┃ ❖ %s
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Peso del mensaje:' : 'Message weight:')} ${chalk.red('%s (%s %sB)')}
┃ ❖ ${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Tipo de mensaje:' : 'Type of message')} ${chalk.bgBlueBright.bold('[%s]')}
╰━━━━━━━━━━━━━━𖡼`.trim(),
//╭──» ${vs} 𓃠 «───✦ 
...
(m.chat.includes("@g.us") ? `${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Grupo:' : 'Group:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}` : `${chalk.white.bold(mbmd.idioma_code === 'es' ? 'Privado:' : 'Private')} ${chalk.greenBright(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
mbmd.idioma_code === 'es' ? await formatMessageTypes(m.mtype) : await formaTxt(m.mtype) || 'Not specified' //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
)
...
async function formatMessageTypes(messageStubType, mbmd) {
switch (messageStubType) {
case 'conversation':
return 'Conversación'
case 'imageMessage':
