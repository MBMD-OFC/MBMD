	...
	let fsizedoc = '1'.repeat(10)    
     ...
     if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
          ...
          forwardedNewsletterMessageInfo: {
            newsletterJid: '120363318353263389@newsletter',
            newsletterName: 'Ver más.', 
            serverMessageId: '' }, 
                ...options
                }
                })
...
export function protoType() {
    ...
    Number.prototype.toTimeString = function toTimeString() {
        ...
        return (
(days ? `${days} ${esmsMT.smsAfkResultTime[0]} ` : '') + (hours ? `${hours} ${esmsMT.smsAfkResultTime[1]} ` : '') +
(minutes ? `${minutes} ${esmsMT.smsAfkResultTime[2]} ` : '') + (seconds ? `${seconds} ${esmsMT.smsAfkResultTime[3]}` : '')).trim()
    }
