...
const src = join(__dirname, '..', 'plugins');
const _svg = readFileSync(join(plugins, 'welcome.svg'), 'utf-8');
const barcode = (data) => {
...
const genSVG = async ({
  wid = '',
  pp = join(plugins, 'src', 'isParticipant.png'),
  title = '',
  name = '',
  text = '',
  background = '',
...
const render = async ({
  wid = '',
  pp = toBase64(readFileSync(join(plugins, 'src', 'isParticipant.png')), 'image/png'),
  name = '',
  title = '',
  text = '',
  background = toBase64(readFileSync(join(plugins, 'files', 'datass', 'Aesthetic_000.jpeg')), 'image/jpeg'),
} = {}, format = 'png') => {
